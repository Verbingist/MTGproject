...
function submitFunction(event) {
    ...
    axios.post('http://localhost:8000/Type', {
        "type_name": createForm.name,
    })
        .then(result => {
            addMesage('Тип успешно создан');
            loadTypes()
        })
        .catch(error => {
...
function deleteButton(type) {
    axios.delete(`http://localhost:8000/Type/${type.type_id}`)
        .then(result => {
            loadTypes();
        })
        .catch(error => {
...
<template>
    ...
    <main class="wrapper">
        ...
        <form @submit="submitFunction" class="inputForm">
            <input class="input" type="text" placeholder="Название типа" v-model="createForm.name">
            <button class="input backButton" type="submit">Подтвердить</button>
        </form>
        <div class="types">
            <div class="type" v-for="type in types">
                <p>{{ type.type_name }}</p>
                <button class="add-button" @click="deleteButton(type)">Удалить</button>
            </div>
        </div>
