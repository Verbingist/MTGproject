...
function loadSubtypes() {
    axios.get(`http://localhost:8000/Subtypes`)
        .then(result => {
            subtypes.value = result.data.subtypes
        })
...
function submitFunction(event) {
    ...
    axios.post('http://localhost:8000/Subtype', {
        "subtype_name": createForm.name,
    })
        .then(result => {
            addMesage('Подтип успешно создан');
            loadSubtypes()
        })
        .catch(error => {
            console.log(error);
            let message = 'Неудачная попытка создания формата';
...
function deleteButton(subtype) {
    axios.delete(`http://localhost:8000/Subtype/${subtype.subtype_id}`)
        .then(result => {
            loadSubtypes();
        })
        .catch(error => {
...
<template>
    ...
    <main class="wrapper">
        <h2>Создать подтип</h2>
        <form @submit="submitFunction" class="inputForm">
            <input class="input" type="text" placeholder="Название подтипа" v-model="createForm.name">
            <button class="input backButton" type="submit">Подтвердить</button>
        </form>
        <div class="subtypes">
            <div class="subtype" v-for="subtype in subtypes">
                <p>{{ subtype.subtype_name }}</p>
                <button class="add-button" @click="deleteButton(subtype)">Удалить</button>
            </div>
        </div>
